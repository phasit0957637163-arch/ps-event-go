...
function AdminLogin({onLogin,onClose}){
  ...
  const [error,setError]=useState("");
  const login=()=>adminId.trim().toLowerCase()==="admin"?onLogin() : setError("เฉพาะ Admin เท่านั้นที่เข้าสู่ระบบได้");
    ...
    <div className="detailColumns">
      <div>
        <section className="panel">
          ...
          <p>{text}</p>
          <div className="photoRow"><div>📷 รูปภาพ Event</div><div>📷 บรรยากาศ</div><div>📷 กิจกรรม</div></div>
          {edit&&<div className="editor"><h3>แก้ไขประสบการณ์</h3><textarea value={text} onChange={e=>setText(e.target.value)}/><input type="file" accept="image/*" multiple/><div><button className="btn ghost" onClick={()=>setEdit(false)}>ยกเลิก</button><button className="btn primary" onClick={()=>setEdit(false)}>💾 บันทึก</button></div></div>}
        </section>
